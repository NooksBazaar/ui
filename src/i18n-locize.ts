// Almost 100% of this file has been taken from https://github.com/UnlyEd/next-right-now/blob/v1-ssr-mst-aptd-gcms-lcz-sty/src/utils/i18nextLocize.ts
//  some changes have been made to update packages used, and to use axios instead of fetch

import isNode from 'detect-node';
import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';
import { DEFAULT_LANG, LANG_EN } from './i18n';
...
export const locizeBackendOptions = {
  ...
  allowedAddOrUpdateHosts: (host: string): boolean => {
    return process.env.NODE_ENV !== 'production'; // We allow any of our development or staging instance to update missing keys
  },
};
...
export async function fetchTranslations(lang: string): Promise<I18nextResources> {
  ...
  try {
    ...
    const defaultLocalesResponse = await Axios.get(locizeAPIEndpoint);
 ...
 */
export function i18nextLocize(lang: string, defaultLocales: I18nextResources) {
